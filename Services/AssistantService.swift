...
final class AssistantService: NSObject, LLMChatting {
    ...
    /// Retrieve accessible spaces based on current campus
    /// - Returns: List of accessible locations
    /// - Throws: NetworkError or DecodingError
    func getAccessibleSpaces() async throws -> [[String: Any]] {
        guard let url = URL(string: "\(backendURL)/api/v1/assistant/accessible-spaces?campus_id=\(campusId)&accessible_only=true") else {
            throw URLError(.badURL)
        }
        ...
        let decoder = JSONDecoder()
        if let json = try decoder.decode([String: Any].self, from: data) as? [String: Any],
    ...
    /// Find nearby spaces from given coordinates
    /// - Parameters:
    ///   - x: X coordinate
    ///   - y: Y coordinate
    ///   - radiusMeters: Search radius in meters (default 50)
    /// - Returns: List of nearby spaces
    /// - Throws: NetworkError or DecodingError
    func findNearbySpaces(x: Double, y: Double, radiusMeters: Double = 50) async throws -> [[String: Any]] {
        guard let url = URL(string: "\(backendURL)/api/v1/assistant/nearby-spaces?campus_id=\(campusId)&location_x=\(x)&location_y=\(y)&radius_m=\(radiusMeters)") else {
            throw URLError(.badURL)
        }
